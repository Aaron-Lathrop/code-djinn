...
const removeEmptyLines = templateContent => templateContent.replace(/\s*[\r\n]/g, '\n').trim();
const addTrailingEmptyLine = templateContent => `${templateContent}\n`;
const trim = templateContent => templateContent.trim();
const formatTemplateContent = (templateContent, ...funcs) => pipe(templateContent, ...funcs);
...
const getTemplateElements = template => {
    return {
        template: formatTemplateContent(getTemplate(template), removeTemplateComments, trim),
...
const getNewFileString = (template, context) => {
    ...
    if (isValidContext) {
        const newFileString = templateVars.reduce((newFileString, currentTemplateVar) => {
            ...
            return formatTemplateContent(newFileString, 
    ...
    // build a single file
const getFileData = async (templateFileName, outputLocation, context) => {
    const templateContents = readFileSync(rootPath(templateFileName));
...
const buildFile = (templateFileName, outputLocation, context) => async () => {
    getFileData(templateFileName, outputLocation, context)
...
const buildDirectoryModuleExports = () => async ({ paths, fileName = 'index.js', rewritable = true }) => {
    paths.forEach(p => {
...
djinn.build = async function build({ templateDir, outputDir, contexts, paths, options, buildSteps }) {
    if (!options) options = {};
    ...
    const buildContexts = options.rewriteAll
        ...
        : contexts;
    const state = { templateDir, outputDir, contexts: buildContexts, paths, options };
    
    for (let i = 0; i < buildSteps.length; i++) {
        if (typeof buildSteps[i] === 'function') await buildSteps[i](state);
    }
}
