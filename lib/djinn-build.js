...
const buildDirectoryModuleExports = async ({ paths, fileName, rewritable }) => {
    paths.forEach(p => {
        getNodeModuleExports(p, fileName)
            ...
            .then(fileString => {
                writeFileSync(path.join(p, fileName), fileString, fileName, rewritable)
            })
...
djinn.build = async function build({ templateDir, outputDir, contexts, paths, options, buildSteps }, ...args) {
    if (!options) options = {};
    ...
    for (let i = 0; i < buildSteps.length; i++) {
        if (buildSteps[i] === djinn.buildDirectoryModuleExports) continue;
        await buildSteps[i](state, ...args);
    }

    if (buildSteps.includes(buildDirectoryModuleExports) && paths && Array.isArray(paths) && paths.length > 0) {
        await buildDirectoryModuleExports({ paths, fileName: 'index.js', rewritable: true }, ...args);
    }
}
