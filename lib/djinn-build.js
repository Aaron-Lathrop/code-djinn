...
const getTemplateElements = template => {
    return {
        template: removeTemplateComments(getTemplate(template)),
...
const build = async ({ templateDir, outputDir, contexts, paths, options }) => {
    ...
    const buildContexts = options.rewriteAll
        ...
        : contexts;
    await buildNewFiles({ templateDir, outputDir, contexts: buildContexts, options });

    if (paths) {
        await buildDirectoryModuleExports({ paths, fileName: 'index.js', rewritable: true});
    }
};
