...
const forEachFileInDirectory = (dir, forEachFile, seedData, ...args) => {
    try {
        ...
        const output = dirnetFiles.reduce((accumulatedData, currentDirnet) => {
            if (currentDirnet.isFile()) {
                ...
                forEachFileInDirectory(path.join(dir, currentDirnet.name), forEachFile, accumulatedData, ...args);
            }
...
const getNewFileString = (template = '', context = {}) => {
    ...
    const newFileString = templateVars.reduce((newFileString, currentTemplateVar) => {
        ...
        if (typeof replaceWith === 'undefined')
            throw Error(`Error creating new file string, context cannot have 'undefined' values
            and context['${currentTemplateVar.cleanVariable}'] was undefined.`)
...
const getNewFileData = (dir = 'templates', newFileData = [], context = {}) => new Promise((resolve, reject) => {
    try {
        const newFiles = forEachFileInDirectory(
            dir,
...
const getModuleExports = (dir, fileName, moduleExports = {}, orginialPath = dir) => new Promise((resolve, reject) => {
    ...
    const modExports = forEachFileInDirectory(
        dir,
...
const buildDirectoryModuleExports = (paths, fileName) => {
    paths.forEach(p => {
...
const build = paths => {
    buildNewFiles();
    buildDirectoryModuleExports(paths, 'index.js');
};
