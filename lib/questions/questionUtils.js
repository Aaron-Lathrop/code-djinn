...
// Formatting
const formatDirectoryAnswer = (answer = '') => {
    if (answer.startsWith('/') || answer.startsWith('\\')) {
        const formattedAnswer = answer.slice(answer.search(/[a-z]/gi));
        console.log(`Folder name cannot start with a "/" or "\\". Updating to "${formattedAnswer}".\n`);
        return formatDirectoryAnswer(formattedAnswer);
    }
    return answer;
};
