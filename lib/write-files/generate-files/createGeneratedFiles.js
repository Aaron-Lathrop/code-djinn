...
const createGeneratedFiles = async () => {
    if (fs.existsSync(path.join(process.cwd(), 'djinn.config.json'))) {
        ...
        const generateFilesPath = path.join(process.cwd(), generateFilesDirectory);

        const generateFilesConfig = require(configFilePath);
        const type = generateFilesConfig && Object.keys(generateFilesConfig)[0] || '';

        const rewriteGeneratedFiles = await questions.generateFilesQuestion(metaDataFileName, configFileName, generateFilesPath);
        if (rewriteGeneratedFiles) writeGeneratedFiles(metaDataFilePath, configFilePath, type);
    } else {
