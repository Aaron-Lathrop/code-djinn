...
const writeGeneratedFiles = (metaDataFilePath, configFilePath) => {   
    ...
    try {
        ...
        if (!fs.existsSync(configFilePath)) {
            logWithColor(colors.FgRed, `Could not find file at "${configFilePath}". Please update the "configFileName" property in "djinn.config.json".`);
        }
        if (!fs.existsSync(path.join(process.cwd(), 'djinn.config.json'))) {
            logWithColor(colors.FgYellow, `Could not find djinn.config.json. Try running 'djinn --init' to create this or move the file to the root of the project.`);
        }
    } catch (err) {
